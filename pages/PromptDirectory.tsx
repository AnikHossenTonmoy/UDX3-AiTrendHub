...
const PromptDirectory = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0B0F19] transition-colors font-sans selection:bg-blue-500/30">
      ...
      {/* CATEGORY FILTER PILLS (Restored) */}
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-4 justify-center">
              {categories.map((cat) => (
                  <button 
                      ...
                      className={`px-5 py-2 rounded-full text-sm font-bold whitespace-nowrap border transition-all ${
                          activeCategory === cat.id 
                          ? 'bg-blue-600 border-blue-500 text-white shadow-lg shadow-blue-600/20 scale-105' 
                          : 'bg-white dark:bg-[#151b2b] border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:border-blue-500/50'
                      }`}
                  >
                      {cat.label}
                  </button>
